...
2026-09-24 的 retry1 回传已确认 950 上编译、32 项校验及 PC 采集通过，源码/指令数据关联可用。
8,576 个已映射计数中第 13 行占 63.83%；另有 50 个未映射计数。Insight 界面与真实通信仍待验证。
上述行号和比例对应旧提交 `fb3ef8a`，新增状态实验见 [重放对照](simt-replay-state.md)，不能沿用旧行号比较。
...
retry1 已完成采样，无需重复运行；用 Insight 打开已有报告核对界面，复现时另选新的输出目录。
采样可能内部重复启动 kernel；`--launch-count=1 --warm-up=0` 不代表只执行一次。
默认模式写入确定结果且输入只读，可以重复执行；不要把该命令直接换成真实通信算子。
...
脚本始终尝试生成小于 5 MB 的 `results/simt-950-first.zip`，包含 manifest、环境/版本日志、
源码快照、带调试信息的可执行文件/对象文件、编译命令及完整 profiler 输出。
请将回传包发回本任务；超限保留原目录并报错。原始包可能含本机路径和设备信息，不要提交到公开仓库。
