...
本次 application 报告已离线核对源码、逐 PC 原始计数及逐行汇总：10,037 个原始计数中 9,999 已映射，38 未映射。
按 ProbeContexts 符号范围 `[0x288, 0x3b0)` 汇总，27 条指令共有 9,999 个样本；其中目标源码行已定位 9,944，未知行 PC `0x390` 有 55。
这 55 个仍属于目标 VF，不能计入其他函数；函数占比分母必须保留未知行。
