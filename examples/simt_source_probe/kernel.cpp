...
__simt_vf__ __aicore__ LAUNCH_BOUND(32) inline void ProbeContexts(
    __gm__ uint32_t* table, __ubuf__ uint32_t* sink, uint32_t steps, uint32_t stateful) {
    // 合成表末尾每线程独占一个可写计数器；只观察本地 GM，不访问通信状态。
    if (stateful) table[4096 + threadIdx.x] += 1U;
...
extern "C" __global__ __aicore__ void akl_simt_probe_kernel(
    GM_ADDR table, GM_ADDR output, uint32_t steps, uint32_t stateful) {
    ...
    Simt::VF_CALL<ProbeContexts>(Simt::Dim3{32, 1, 1},
        ...
        reinterpret_cast<__ubuf__ uint32_t*>(values.GetPhyAddr()), steps, stateful);
...
extern "C" void launch_probe(void* stream, void* table, void* output, uint32_t steps, uint32_t stateful) {
    // 首个能力探针明确只启动一个逻辑 AIV，不代表设备总核数。
    akl_simt_probe_kernel<<<1, nullptr, stream>>>(
        static_cast<uint8_t*>(table), static_cast<uint8_t*>(output), steps, stateful);
}
